"""
Select a threshold for each group to ensure enough relevant items from each group using the monotone rule based on the
...
if __name__ == "__main__":
    ...
    t_hat_clicked = None
    for t in range(t_max_clicked - 1, 0, -1):
        # W_max = min(t_max_clicked + t_max_not_clicked, np.sqrt(n * 1. / t))
        u_lower_bound = calculate_lower_bound(user_feedback, t=t, group=1, alpha=args.alpha, W_max=args.W_max)
        if u_lower_bound < args.k_clicked:
            t_hat_clicked = t + 1
            break
    if t_hat_clicked is None:
        print("every threshold for the clicked group can guarantee the lower bound is grater than target number of "
              "relevant items")
        t_hat_clicked = 1
    if t_hat_clicked == t_max_clicked:
    ...
    for t in range(t_max_not_clicked - 1, 0, -1):
        # W_max = min(t_max_clicked + t_max_not_clicked, np.sqrt(n * 1. / t))
        u_lower_bound = calculate_lower_bound(user_feedback, t=t, group=0, alpha=args.alpha, W_max=args.W_max)
        if u_lower_bound < args.k_not_clicked:
            t_hat_not_clicked = t + 1
            break
    if t_hat_not_clicked is None:
        print("every threshold for the not clicked group can guarantee the lower bound is grater than target "
              "number of relevant items")
        t_hat_not_clicked = 1
    if t_hat_not_clicked == t_max_clicked:

"""
Select top items from each group so that the sum of the Platt scaling calibrated prediction scores
(calibrated per group) is greater than the target number of qualified items on average over the calibration set
"""
...
if __name__ == "__main__":
    ...
    for query in user_feedback:
        ...
        for j in range(scores.size):
            if query["groups"][j]:
                new_scores.append(scores_clicked[j])
            else:
                new_scores.append(scores_not_clicked[j])
        query["scores"] = np.array(new_scores)
